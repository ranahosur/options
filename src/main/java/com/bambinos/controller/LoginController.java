...
public class LoginController extends LoginRegistrationBaseController {
  ...
  public ModelAndView loginProcess(HttpServletRequest request, HttpServletResponse response,
    ...
    login = userService.validateUser(login);

    if (null != login ) {
//      logger.debug("User is validated , loginProcess redirecting to Welcome for firstname "+ login.getFirstName() + " lastname "+ user.getLastName() + " houseNo "+ user.getAddressLine1() + " street "+ user.getAddressLine2() + " city "+ user.getCity());
//      return showWelcomePage(request,user);
  ...
  public ModelAndView resetPasswordVerify(HttpServletRequest request, HttpServletResponse response,
                                          @ModelAttribute("login") Login login) {
    String username = null;//login.getEmail();
